...
function prepareDiagonalData(sortedData, options) {
  ...
  const firstStage = _.min(_.map(sortedData, 'stage'));
...
function prepareSimpleData(sortedData, options) {
  const timeInterval = options.timeInterval;
  const grouped = groupData(sortedData);
  const stages = _.map(sortedData, 'stage');
  ...
  return {
    ...
    link($scope, element) {
      ...
      function updateCohort() {
        ...
        const columnNames = _.map($scope.queryResult.getColumns(), 'name');
        if (
  ...
  return {
    ...
    link: ($scope) => {
      ...
      function refreshColumns() {
        ...
        $scope.columnNames = _.map($scope.columns, 'name');
      }
