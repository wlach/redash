...
class Parameters {
  ...
  parseQuery() {
    ...
    try {
      ...
      parameters = map(this.query.options.parameters, 'name');
    }
    return parameters;
  }
  ...
  getMissing() {
    return map(filter(this.get(), p => p.value === null || p.value === ''), 'title');
  }
  ...
  getValues() {
    ...
    return zipObject(map(params, 'name'), map(params, 'value'));
  }
}
